...
class Wallet:
    ...
    def __init__(self, public_key: str = None, private_key: str = None, auction_endpoint: str = "http://localhost:3000/"):
        if not private_key and not public_key:
            raise ValueError("Can't instanciate a Wallet without a public or private key")

        self.private_key = private_key
        self.public_key = public_key
        self.auction_endpoint = auction_endpoint
    ...
    def sign_bid_data(
        ...
        signature = self.get_signature(domain, message_to_sign, types)
    ...
    def get_signature(self, domain: Domain, message_to_sign: MessageToSign, types: dict) -> str :
    ...
    def submit_bid(self, domain: Domain, bidId: int, quantity: int, price: int, isBuying: bool, expiry: int, nonce: int, types: dict = ORDER_TYPES): 
        order : MessageToSign = MessageToSign(
            bidId,
            self.public_key,
            quantity,
            price,
        ...
        signature = self.get_signature(domain, order, types)
        req = requests.post(self.auction_endpoint + 'api/auction/createOrEditBid',
                            json={"order": order.__dict__, "signature": Signature(signature[0], Web3.toHex(signature[1].to_bytes(32, 'big')), Web3.toHex(signature[2].to_bytes(32, 'big'))).__dict__})
